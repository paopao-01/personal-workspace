...
test('V0.3 starts a project mock interview without changing the project', async ({ page, request }) => {
  ...
  await expect(page.getByText('信息不足：当前仅有 1 条已完成评分；至少需要 2 条才显示平均分。')).toBeVisible()
  await expect(page.getByText('分布：1 分 0 条 · 2 分 0 条 · 3 分 0 条 · 4 分 1 条 · 5 分 0 条')).toBeVisible()
