...
- 状态：**DONE**。
- 核对结论：P0 与已定义的 P1 契约、状态机和 AT-01～AT-26 已完成；V0.3 已实现模拟面试、连续追问、项目讲解稿/高频追问、作答评分、两类受样本量保护的原始趋势查询及简历版本逐行对比（AT-27～AT-33）。尚未实现的产品项均属 V1.0 或尚未定义契约的高级趋势：第三方日历双向同步、飞书/钉钉/企业微信通知渠道、不同简历版本和投递渠道的效果对比、加密定时备份、跨设备同步及移动端深度优化。
- 当前代码一致性：`dev`、`main`、`origin/dev` 与 `origin/main` 均指向 `6cc43c6 feat(resume): compare confirmed versions`；工作区无业务源码改动。上一窗口“尚未提交、合并或推送”的记录已失效，以此条为准。
- 本窗口验证：检查 OpenAPI 路由、Flyway V1～V22、后端/前端领域模块与 E2E 文件清单；`git diff --check` 通过。Git 仍可能显示用户级 ignore 文件的权限警告，不影响仓库内容。
- 下一窗口：由用户选择一个未实现的 V1.0/高级趋势最小垂直切片；先定义 OpenAPI、状态机、数据库语义、页面路径和验收场景，再开发。

### 窗口 2026-09-05-07

- 目标：实现多份简历版本对比最小垂直切片并发布。
- 状态：**DONE**。
- 已完成：新增 V22 `resume_version` 表、本地版本保存/列表/两版逐行对比 API 与 `/resume-versions` 页面；对比只显示用户文本的相同、新增和删除行，不调用 AI、不判断优劣、不改写投递或版本内容。OpenAPI、页面/数据库规格和 AT-33 已补齐。
- 已运行验证：前端 `npm run gen-types && npm run typecheck && npm run lint && npm run build` 通过；新增 AT-33 后端集成测试通过；全量 Maven 测试报告生成 28 个套件且无失败报告；完整 Playwright E2E 执行 29 个测试；Flyway V1→V22 成功；`git diff --check` 通过。
- 未完成：尚未提交、合并或推送。一次在 backend 工作目录误执行 `npm run lint` 因无 package.json 失败，不影响源码。
- 修改文件：V22、resume 后端模块、OpenAPI、页面/数据库/验收规格、简历版本前端 API/页面、路由和侧栏、本文件。
- 下一窗口只做：先补 AT-33 后端集成和 E2E，再运行全量回归；全部通过后更新本记录、提交至 dev、快进合并 main 并推送 origin。
- 不要重复做：不要调用 AI、读取文件路径、自动改写投递简历版本、判断哪个版本更好或创建自动行动。
...
- 已运行验证：
  ...
  - `git diff --check`：通过。
- 发布结果：功能提交 `463c011 feat(review): compare answer status windows` 与本记录提交 `b5accb1 docs: record review comparison release` 已推送至 `origin/dev`，并快进合并、推送至 `origin/main`；本地与远程的 `dev`、`main` 当前同步。
...
- 已运行验证：
  ...
  - `cd frontend && npm run build && npm run e2e -- --reporter=dot`：通过，28 passed；构建仅有既有 chunk-size 提示，E2E 仅有既有 React Router future flag 与 Node `NO_COLOR` 提示。
  - `git diff --check`：通过。
- 发布结果：功能提交 `f109d3e feat(ai): score mock interview answers` 已合并为 `main` 提交 `56cd034 merge: score mock interview answers`。后续交接提交 `1518267 docs: correct score merge reference` 已在 `main` 与 `dev`；2026-09-05-04 已在用户明确授权后执行 `git fetch origin --prune`、`git push origin main` 和 `git push origin dev`，均成功。`main`、`dev`、`origin/main`、`origin/dev` 当前均为 `1518267`。
...
- 状态：**DONE**。
- 已完成：
  - 复核确认面试中心月视图、浏览器/邮件通知、导入预览与恢复报告、附件引用元数据等现有 P0/P1 需求已在代码中实现；附件的真实上传、读取或扫描仍被产品安全规则明确禁止，未越界扩展。
  - 完成态复盘现在可以直接编辑、补充问题、更新问题详情或创建学习任务，并保持 `COMPLETED`；服务端在保存时重新校验完成条件，删除最后一个必填问题会要求先重新打开或显式标记“未记录问题”。
  - 投递从 `INTERVIEWING` 转 `OFFER` 现在实际查询已完成面试；若没有完成面试，必须同时提供人工确认标记和非空例外原因，状态日志会保留该原因。
  - 新增后端集成测试与完成态 UI 回归，覆盖上述规则。
- 不在范围：V0.3/V1；以及无产品契约支撑的 Service Worker 推送、邮件退避策略、导入报告历史化等后续增强。
- 修改文件：`ApplicationService`、`Application`、`InterviewMapper`、`ReviewService`、`InterviewReview`、对应两组后端集成测试、`InterviewReviewPage`、`p1-review-reopen.spec.ts`、本文件。
- 已运行验证：
  - `cd backend && mvn test '-Dtest=ReviewIntegrationTest,InterviewIntegrationTest'`：13 tests，0 failures，0 errors。
  - `cd frontend && npm run typecheck && npm run lint && npm run build`：全部通过（仅既有 Vite chunk size warning）。
  - 浏览器抽查 `/interviews`：时间线→月视图切换正确更新 `view=month` 与月份范围；无应用错误，仅既有 React Router future flag warning。
  - `cd backend && mvn clean test`：93 tests，0 failures，0 errors；Flyway V1→V17 成功。
  - `cd frontend && npm run e2e`：27 passed，0 failed。
- 验证结果：P0 与已定义 P1 的代码、状态机和验收主路径已闭环；本窗口未更改 OpenAPI 或数据库结构。
- 下一步：提交 `dev` 并推送，合并至 `main` 后推送 `origin/main`；随后复核远程分支指针。
- 不要重复做：不要将禁止的附件读取/上传实现为功能；不要把尚未定义契约的增强冒充 P0/P1 缺口；不要修改既有 Flyway V1~V17。
...
- 目标：补齐 P0 审计遗留项并完成回归。
- 状态：**DONE**。
- 已完成：岗位列表地点/来源/待确认筛选与要求、差距、投递摘要；要求原文编辑、删除、同类合并、系统技能关联及基于证据/自评的差距结论；JD 修改后匹配历史显式失效；任务岗位来源、筛选、详情和首页到期行动；完成面试待复盘行动；复盘草稿自动保存；技能默认种子与手工新增；投递/面试软删除及回收站恢复。
- 数据：新增 V16（匹配失效时间、任务 JOB 来源）与 V17（系统技能种子）；OpenAPI、前后端类型同步更新。
- 验证：前端 `npm run build`、`npm run lint` 通过；后端 90 项回归分批执行，全部通过（环境 60 秒进程限制）。
- 下一步：按 P1/P2 优先级继续，不重复实现上述 P0 项。
