...
- passphrase 强度门槛（强制）在内存校验，不新增表/列/迁移：创建备份与武装调度的 service 入口评估 passphrase 强度（与前端同一算法，阈值弱<40），弱口令返回 400 不落盘/不武装；passphrase 与派生密钥**永不**持久化（与既有硬规则一致），强度评估仅在调用栈内进行，passphrase 不落盘、不进日志、不回显、不新增评估端点。恢复端点不强制（passphrase 已与备份绑定）。恢复成功后弱口令重设提示亦在内存校验，不新增表/列/迁移：恢复成功后对本次 passphrase 做一次内存强度评估，仅弱（score<40）置响应 `passphraseResetRecommended=true`（非阻塞，建议用户用强口令新建备份替换，系统无全局 passphrase 可重设），强/中为 false；passphrase 与派生密钥**永不**持久化、不回显、不进日志，响应仅返回布尔值不回显 score/level；标准数据恢复不评估，该字段为 null。
