...
- 全量审计日志查询（`GET /audit-logs`）只读，复用 V1 既有 `audit_log` 表，**不新增表/列/迁移/索引**：分页查询全量审计记录，支持可选 `action` 与 `resourceType` 过滤（均可空、可单独或组合，按字符串精确匹配，空=不过滤），承接 `GET /backups/orphans/audit`（仅 `BACKUP_ORPHAN_CLEANED` 的备份域便捷入口，不废弃）作为跨域通用查询入口（`AuditLogMapper` 在 `selectPageByAction`/`countByAction` 之外新增只读 `selectPage(action, resourceType, pageSize, offset)` + `count(action, resourceType)`，动态 SQL 参数 null 不加条件，仅 SELECT 不违背仅追加语义），按 `occurred_at DESC` 排序（ISO-8601 字典序与时间序一致），`offset=(page-1)*pageSize`；V1 表无二级索引，带可选 `WHERE action=? AND resource_type=?` 的全表扫描，本地单用户审计量小可接受，不新增索引迁移。响应暴露 `id`/`resourceType`/`resourceId`/`action`/`reason`/`occurredAt`（全量查询 resourceType 不固定故返回；省略恒 null 的快照字段），不含 passphrase；只读不写、不需 `X-Confirm-Permanent-Delete`、不需 `Idempotency-Key`、不动任何业务表。`action=BACKUP_ORPHAN_CLEANED` 时返回与 `GET /backups/orphans/audit` 相同的记录集（字段含 resourceType，其余语义一致）。全量查询只读暴露既有写入值，不新增 action：`SECONDARY_APPLICATION_CONFIRMED`（`resourceType=APPLICATION`）、`REQUIREMENT_MERGED`/`REQUIREMENT_UPDATED`/`REQUIREMENT_DELETED`（`resourceType=JOB_REQUIREMENT`）、`BACKUP_ORPHAN_CLEANED`（`resourceType=BACKUP_FILE`）、`BACKUP_DELETED`/`BACKUP_PURGED_BY_AGE`/`BACKUP_PURGED_BY_COUNT`（`resourceType=BACKUP_RECORD`，单条删除/按龄清理/按数量保留清理写入）。
