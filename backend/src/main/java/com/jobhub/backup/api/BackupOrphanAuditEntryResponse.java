 ...
 * before/afterSnapshotJson（孤儿清理不存快照，无展示价值）；freedBytes 为结构化字段（被删文件字节数），
 * reason 不含字节数子串。审计记录从不存 passphrase，本响应不含 passphrase。
 */
public record BackupOrphanAuditEntryResponse(
		String id, String resourceId, String action, String reason, Long freedBytes, String occurredAt) {
	...
	static BackupOrphanAuditEntryResponse from(AuditLogEntry entry) {
		return new BackupOrphanAuditEntryResponse(
				...
				entry.getReason(), entry.getFreedBytes(), entry.getOccurredAt());
	}
}
