 ...
 * 含 freedBytes（仅 BACKUP_ORPHAN_CLEANED 行有值，其余 action 为 null）；
 * 省略 before/afterSnapshotJson（当前所有审计记录快照恒为 null，无展示价值，与既有审计查询端点一致）。
 * 审计记录从不存 passphrase，本响应不含 passphrase。
 */
public record AuditLogEntryResponse(
		String id, String resourceType, String resourceId, String action, String reason,
		Long freedBytes, String occurredAt) {

	static AuditLogEntryResponse from(AuditLogEntry entry) {
		return new AuditLogEntryResponse(
				entry.getId(), entry.getResourceType(), entry.getResourceId(),
				entry.getAction(), entry.getReason(), entry.getFreedBytes(), entry.getOccurredAt());
	}
}
