 ...
 * freedBytesMin/freedBytesMax 过滤 + occurred_at DESC 排序，复用 V1 既有 audit_log 表（V26 补 occurred_at 单列索引、
 * V28 补 action+occurred_at 复合索引）无新增迁移。
 */
@Mapper
public interface AuditLogMapper {
	...
	/**
	 * 全量审计日志分页查询（只读）：action、resourceType、from、to、hasFreedBytes、freedBytesMin、freedBytesMax 均
	 * 可选，null 或空（hasFreedBytes 为 false）时不加条件返回全量；action/resourceType 按字符串精确匹配，
	 * from/to 按 occurred_at 时间范围过滤（ISO-8601 UTC 字符串比较，字典序与时间序一致），hasFreedBytes=true 只返回
	 * freed_bytes IS NOT NULL 的记录，freedBytesMin/freedBytesMax 按 freed_bytes 数值范围过滤（闭区间，对 NULL 行
	 * 求值为 NULL→false 自动排除非孤儿清理行，隐式含 NOT NULL 语义）。七者可单独或任意组合使用。按 occurred_at DESC 排序。
	 */
	@Select("<script>" +
	...
	/** 全量审计日志计数（只读）：action、resourceType、from、to、hasFreedBytes、freedBytesMin、freedBytesMax 均可选，
	 *  null 或空（hasFreedBytes 为 false）时不加条件计数全量。 */
	@Select("<script>" +
	...
	/**
	 * 全量审计日志导出查询（只读）：action、resourceType、from、to、hasFreedBytes、freedBytesMin、freedBytesMax 均可选，
	 * null 或空（hasFreedBytes 为 false）时不加条件返回全量；条件与 {@link #selectPage} 完全一致，仅去掉
