-- 扩展既有 AI 任务枚举；SQLite 需重建带 CHECK 的表，保留所有历史任务与候选。
PRAGMA foreign_keys = OFF;
ALTER TABLE ai_job_item RENAME TO ai_job_item_v18_old;
ALTER TABLE ai_job RENAME TO ai_job_v18_old;
CREATE TABLE ai_job (
  id TEXT PRIMARY KEY,
  job_type TEXT NOT NULL CHECK (job_type IN ('JD_EXTRACTION', 'RESUME_DRAFT', 'QUESTION_CLASSIFICATION', 'ANSWER_QUALITY_ANALYSIS', 'TASK_SUGGESTION', 'MOCK_INTERVIEW')),
...
INSERT INTO ai_job SELECT * FROM ai_job_v18_old;
DROP TABLE ai_job_v18_old;
...
INSERT INTO ai_job_item SELECT * FROM ai_job_item_v18_old;
DROP TABLE ai_job_item_v18_old;
