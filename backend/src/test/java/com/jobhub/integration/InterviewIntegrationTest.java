...
class InterviewIntegrationTest extends AbstractIntegrationTest {
    ...
    @Test void offerRequiresCompletedInterviewOrExplicitReasonedException(){
        String withoutCompletedInterview=createApplicationAt("INTERVIEWING");
        ResponseEntity<String> rejected=restTemplate.exchange(url("/applications/"+withoutCompletedInterview+"/transition"),HttpMethod.POST,
            TestFixtures.httpWithHeaders(TestFixtures.transitionBody("OFFER",null,null),"Idempotency-Key",TestFixtures.newKey(),"If-Match-Version","3"),String.class);
        assertThat(rejected.getStatusCode()).isEqualTo(HttpStatus.UNPROCESSABLE_ENTITY);
        ResponseEntity<String> missingReason=restTemplate.exchange(url("/applications/"+withoutCompletedInterview+"/transition"),HttpMethod.POST,
            TestFixtures.httpWithHeaders(TestFixtures.transitionBody("OFFER",null,true),"Idempotency-Key",TestFixtures.newKey(),"If-Match-Version","3"),String.class);
        assertThat(missingReason.getStatusCode()).isEqualTo(HttpStatus.UNPROCESSABLE_ENTITY);
        ResponseEntity<String> exception=restTemplate.exchange(url("/applications/"+withoutCompletedInterview+"/transition"),HttpMethod.POST,
            TestFixtures.httpWithHeaders(TestFixtures.transitionBody("OFFER","招聘方书面确认免面试",true),"Idempotency-Key",TestFixtures.newKey(),"If-Match-Version","3"),String.class);
        assertThat(exception.getStatusCode()).isEqualTo(HttpStatus.OK);

        String withCompletedInterview=createApplicationAt("RESUME_PASSED");
        String interviewId=JsonProbe.str(createInterview(withCompletedInterview,"2026-09-10T10:00:00Z").getBody(),"id");
        long interviewVersion=JsonProbe.lng(readInterview(interviewId),"version");
        ResponseEntity<String> completed=restTemplate.exchange(url("/interviews/"+interviewId+"/complete"),HttpMethod.POST,
            TestFixtures.httpWithHeaders("{\"result\":\"PASSED\"}","Idempotency-Key",TestFixtures.newKey(),"If-Match-Version",String.valueOf(interviewVersion)),String.class);
        assertThat(completed.getStatusCode()).isEqualTo(HttpStatus.OK);
        String application=restTemplate.getForEntity(url("/applications/"+withCompletedInterview),String.class).getBody();
        ResponseEntity<String> offer=restTemplate.exchange(url("/applications/"+withCompletedInterview+"/transition"),HttpMethod.POST,
            TestFixtures.httpWithHeaders(TestFixtures.transitionBody("OFFER",null,null),"Idempotency-Key",TestFixtures.newKey(),"If-Match-Version",JsonProbe.str(application,"version")),String.class);
        assertThat(offer.getStatusCode()).isEqualTo(HttpStatus.OK);
        assertThat(JsonProbe.str(offer.getBody(),"status")).isEqualTo("OFFER");
    }
